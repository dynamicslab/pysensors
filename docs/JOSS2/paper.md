...
Pysensors implements `Sparse Sensor Placement Optimization for Reconstruction (SSPOR)`[manohar2018data] of full fields $\mathbf{x}\in\reals^n$ from $p$ noise-corrupted sensor measurements $\mathbf{y}\in\reals^p$
$$
    \mathbf{y} =  \mathbb{S} \mathbf{x} + \boldsymbol\eta,
$$
where $\boldsymbol{\eta}$ consists of zero-mean, Gaussian independent and identically distributed (i.i.d.) components, and $\mathbb{S} \in \reals^{p\times n}$ is the desired sensor (measurement) selection operator.
...
High dimensional field dynamics $\mathbf{x}\in\reals^n$ can be represented as a linear combination of spatial basis modes $\boldsymbol{\Psi}$ weighted by time-varying coefficients $\mathbf{a}$
...
When a user specifies $p$ sensors, SSPOR first fits a basis $\bTi_r$ to the data and optimizes sensor placement by minimizing the following objective function:
$$
...
![A flowchart to suggest which method to use](docs/JOSS2/Fig2.jpeg "When selecting a sensing method in PySensors, consider your primary objective: For field reconstruction in standard settings, use QR with Identity or SVD basis. For classification tasks, leverage SVD basis with SSPOC optimizer. When facing spatial constraints, choose GQR optimizer. For under-sampling (p < r) and over-sampling cases (p > r)scenarios , select TPGR optimizer. In noisy environments enable uncertainty quantification for robust results.")
